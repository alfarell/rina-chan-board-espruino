...
const mode_list = [
  {
    ...
    actions: {
      D17: () => {
        ...
        if (key === 'blush') {
          if (active_key_expression[key] >= 1) {
            active_key_expression[key] =
              typeof active_key_expression[key] === 'number'
                ? (active_key_expression[key] || expresionPreset[key].length) -
                  1
                : expresionPreset[key].length;
          } else {
            active_key_expression[key] = false;
          }
        } else {
          if (active_key_expression[key] >= 1)
            active_key_expression[key] = active_key_expression[key] - 1;
      ...
      D18: () => {
        ...
        prev_active_key_expression[key] = active_key_expression[key];
        if (key === 'blush') {
          if (active_key_expression[key] >= expresionPreset.blush.length - 1)
            active_key_expression[key] = false;
          else
            active_key_expression[key] =
              typeof active_key_expression[key] === 'number'
                ? (active_key_expression[key] || 0) + 1
                : 0;
        } else {
          active_key_expression[key] =
            (active_key_expression[key] + 1) % expresionPreset[key].length;
        }
      ...
      D19: (e) => {
        if (!e.state) {
          if (e.time - e.lastTime < 2) {
            expression_mode =
              (expression_mode + 1) % expression_mode_list.length;
          }
          const key = expression_mode_list[expression_mode];
...
function runActiveAction(e) {
  const action =
    mode_list[active_mode] &&
    mode_list[active_mode].actions &&
    mode_list[active_mode].actions[e.pin];
  action && action(e);
}
...
function setLed() {
  ...
  if (active_key_expression.eyes !== prev_active_key_expression.eyes) {
    expresionPreset.eyes[active_key_expression.eyes].data.forEach((num) =>
      setColor(num, RINA_COLOR)
    );
  }
  if (active_key_expression.mouth !== prev_active_key_expression.mouth) {
    expresionPreset.mouth[active_key_expression.mouth].data.forEach((num) =>
      setColor(num, RINA_COLOR)
    );
  }
  if (active_key_expression.blush !== prev_active_key_expression.blush) {
    if (
      typeof active_key_expression.blush === 'number' &&
      expresionPreset.eyes[active_key_expression.eyes].withBlush
    ) {
      expresionPreset.blush[active_key_expression.blush].data.forEach((num) =>
        setColor(num, RINA_COLOR)
      );
    } else {
